...
class LexemesRecyclerAdapter(private var lexeme: List<Lexeme>) : RecyclerView.Adapter<LexemesRecyclerAdapter.LexemeViewHolder>() {

    private lateinit var mItemClickListener: OnItemClickListener

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LexemeViewHolder {
        val view: View = LayoutInflater.from(parent.context).inflate(R.layout.item_favorite_lexeme, parent, false)
        return LexemeViewHolder(view, mItemClickListener)
    }

    override fun getItemCount(): Int = lexeme.size

    override fun onBindViewHolder(holder: LexemeViewHolder, position: Int) = holder.bind(lexeme[position])

    class LexemeViewHolder(itemView: View, private val mItemClickListener: OnItemClickListener) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
